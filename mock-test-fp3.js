...
const dummyData = {
  machineId: 'fire-pump-3',
  inspectionDate: '2026-06-20',
  generalData: {
    location: 'อาคาร ทดสอบ FP3',
    model: 'FP3-MODEL',
    serialNumber: 'FP3-SERIAL',
    fuelBefore: 300,
    fuelAfter: 290,
    runDurationMins: 30,
  ...
  afterRun: {
    comment: 'ทดสอบ FP3',
    inspectedBy: 'ทดสอบ FP3',
  },
};

async function runTest() {
  const templatePath = path.join(__dirname, 'templates', 'Template_FPG.xlsx');
  const outputPath = path.join(__dirname, 'OUTPUT_TEST_FP3.xlsx');
  try {
    const buffer = await generateExcelReport(dummyData, templatePath);
    fs.writeFileSync(outputPath, buffer);
    console.log('🎉 fire-pump-3 สำเร็จ:', outputPath);
  } catch (error) {
    console.error('❌ fire-pump-3 ล้มเหลว:', error.message);
    process.exitCode = 1;
  }
}
runTest();
